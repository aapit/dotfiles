...
#+end_src
**** link faces
Roam links krijgen een eigen kleur zodat ze te onderscheiden zijn van http(s) links.
#+begin_src emacs-lisp :tangle "config.el" :results silent
(after! org
...
(custom-set-faces
 '(org-roam-link ((t (:foreground "#9966ff"))))
...
#+END_SRC
*** encryption
Configuratie voor =org-crypt=. Ik gebruikte eerst alleen compleet encrypted org files (=*.org.gpg=), maar ik wil ook alleen node content (itt metadata) kunnen encrypten zodat de doorzoekbaarheid overeind blijft.
#+begin_src emacs-lisp
(after! org
    (require 'org-crypt)
